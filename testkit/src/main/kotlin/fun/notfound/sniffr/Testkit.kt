...
object Testkit {
    fun standard(sniffr: Sniffr<JsonElement>) = freeSpec {
        fun FreeSpecTerminalScope.enforce(
            lhs: String,
            rhs: String,
            head: Delta, vararg tail: Delta
        ) {
            enforce(lhs, rhs, head, tail = tail, sniffr)
        }

        val sniff = sniffr::sniff
        ...
        "mixed" - {
            ...
            "replace - type change" - {
                listOf(
                    "Null -> Int"    to """{ "a": null }""" to """{ "a": 1 }""",
                    "Null -> Object" to """{ "a": null }""" to """{ "a": {} }""",
                    "Null -> Array"  to """{ "a": null }""" to """{ "a": [] }""",
    ...
    fun lenient(sniffr: Sniffr<JsonElement>) = freeSpec {
        "lenient" - {
            "same" - {
                "shallow" {
                    val root = parse("""[ 1 ]""")
